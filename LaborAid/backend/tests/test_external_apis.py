...
@pytest.mark.asyncio
async def test_list_external_apis_empty(client, admin_auth_headers):
    """Listing external APIs when none exist should return an empty list."""
    response = await client.get("/api/v1/external-apis", headers=admin_auth_headers)
    assert response.status_code == 200
    assert isinstance(response.json(), list)


@pytest.mark.asyncio
async def test_create_external_api(client, admin_auth_headers):
...
@pytest.mark.asyncio
async def test_create_external_api_invalid_json(client, admin_auth_headers):
...
@pytest.mark.asyncio
async def test_update_external_api(client, admin_auth_headers):
...
@pytest.mark.asyncio
async def test_delete_external_api(client, admin_auth_headers):
...
@pytest.mark.asyncio
async def test_delete_nonexistent_external_api(client, admin_auth_headers):
...
@pytest.mark.asyncio
async def test_get_presets(client, admin_auth_headers):
...
@pytest.mark.asyncio
async def test_toggle_external_api(client, admin_auth_headers):
