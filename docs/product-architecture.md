...
> **原则**：保留登录与历史；保留查法、文书等原有能力；不做社区发帖  
> **⚠️ 时效**：下文「不做管理员端」已过时 — 当前已有完整管理端（`/admin/*`）、维权助手编排、比赛四步演示；**路由与功能以 [README](../README.md) 为准**。

---

## 1.4 当前实现（2026，相对 v1 设计稿的变更）

| 主题 | v1 设计稿 | 当前实现 |
|------|-----------|----------|
| 维权入口 | 首页单一「说说你的情况」 | **EntryGate**：专项维权（结构化表单）/ 普通入口（自由文本） |
| 专项通道 | 独立 `/channels` 三页 | 收敛至首页 `?intake=special`；旧 URL 重定向或 `ChannelLegacyRedirect` |
| 维权指引 | 案由步骤 + 外链 | **`/guidance` = 办事资源**：官方平台条 + 热线；案由步骤在 intake 计划内 |
| 数据闭环 | session 为主 | 建案时 `POST /intake/create-case` 写入 `case.ai_snapshot.intake`；readiness 可读快照 |
| 法律知识库 | 用户端 `/knowledge` | 重定向首页；管理端维护 |

**信息架构（当前）**

```
登录 → 服务首页 (/)
         ├── 专项维权 intake（农民工 / 实习生·试用期 / 女职工）
         ├── 普通入口 intake（文字 + 图）
         └── 推荐计划 → 建案 → 证据 / 文书 / 报告
         │
         ├── 办事资源 (/guidance)     ← 权威外链、31 省平台
         ├── 我的记录 (/records)
         └── 法律工具箱（cases、evidence、documents…）
```
...
| 页面 | 路由建议 | 说明 |
|------|----------|------|
| 服务首页 | `/` | 维权 intake（专项 / 普通）+ 工具入口 + 案件协作 |
| 我的记录 | `/records` | Tab：案件 / 文书 / 研究 / 最近工具 |
| 办事资源 | `/guidance` | 官方平台条、一键办事、热线（非案由向导） |
| （已收敛） | `/channels` | 重定向首页专项 intake；`:channelId` 兼容旧链接 |

### 4.2 工具层页面（保留，不改路径）

| 模块 | 路由 | 能力 |
|------|------|------|
| 检索法规 | `/search` | 法规 / 案例 / HERA 检索 + AI 摘要 |
...
> **当前**：`/guidance` 为办事资源页（官方平台条 + 热线），案由步骤在首页 intake 维权计划内。以下为 v1 设计稿中的案由向导思路，部分逻辑已迁移至 intake。
