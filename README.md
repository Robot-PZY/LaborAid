...
| 访问 | 地址 |
|------|------|
| 🖥️用户端 | http://127.0.0.1:5320 |
| 📖API 文档 | http://127.0.0.1:8010/api/docs |
| 🔐管理端 | http://127.0.0.1:5320/login?portal=admin |
...
| ⚛️前端 | React 19 · TypeScript · Vite · Tailwind | 用户端 + 管理端 SPA |
| 🤖AI | LLM + 视觉 OCR（可配置） | 案情分析、文书、证据识别 |
| 🗃️数据 | SQLite / PostgreSQL | 开发默认 SQLite，生产可切 Postgres |
...
> 默认端口 **5320 / 8010**，避免与常见 Vite(5173)、Uvicorn(8000) 冲突；`dev.ps1` 仅清理这两个端口上的旧进程。
...
| 服务 | 地址 |
|------|------|
| 🖥️前端 | http://127.0.0.1:5320 |
| ⚙️后端 API | http://127.0.0.1:8010 |
...
| 项 | 值 |
|----|-----|
| 入口 | http://127.0.0.1:5320/login?portal=admin |
